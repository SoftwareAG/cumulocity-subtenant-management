...
export class FakeMicroserviceService {
    ...
    private async getMicroserviceSubscriptions(bootstrapCredentials: ICredentials, baseUrl?: string): Promise<ICredentials[]> {
        return Client.getMicroserviceSubscriptions(bootstrapCredentials, baseUrl);
    }

    private async performStepForEveryTenant<T>(
        app: IApplication, step: (app: IApplication, tenant: ITenant) => Promise<T>,
        subscribedOnesOnly: boolean, unsubscribedOnesOnly: boolean
    ): Promise<T[]> {
